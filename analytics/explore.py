...
# ---------------- 
#
# The goal here is to have a list of lists, where each 


# Construct.... Corpus of documents? 
list_of_decklists = [[]]

# Iterate through each unique deck_id
for id in select_deck_ids:

    # Filter for a given deck
    filter_matching_ids = (df['deck_id'] == id)
    given_deck = df[filter_matching_ids]
  
    deck_sublist = []
    for card in given_deck['card_name']:
        deck_sublist.append(card)
    list_of_decklists.append(deck_sublist)

#print(dir(list_of_decklists))

for item in list_of_decklists:
    print('\n\n', item, '\n\n')
...
# Pandas method of querying
query = "SELECT cards.card_name, cards.count_maindeck, cards.count_sideboard, decks.deck_format FROM scryfall.decks WHERE deck_format = Historic INNER JOIN scryfall.cards ON cards.deck_id = decks.id;"
